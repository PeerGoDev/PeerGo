...
const (
	MaxActivePeerLimit     = 200
	MaxUserActivePeerLimit = 200
)

var (
	activePeerUserIDPattern   = regexp.MustCompile(`^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$`)
	activePeerInfoHashPattern = regexp.MustCompile(`^[0-9a-f]{40}$`)
)
...
// UserActivePeer is the bounded cross-swarm view used for a member's own
// profile and the authorized staff account view. Torrent size comes from the
// signed control snapshot, not from peer-supplied announce values.
...
type UserActivePeerPage struct {
	UserID      string           `json:"user_id"`
	GeneratedAt time.Time        `json:"generated_at"`
	Items       []UserActivePeer `json:"items"`
	Truncated   bool             `json:"truncated"`
}

func (page UserActivePeerPage) Valid(expectedUserID string, limit int) bool {
	if page.UserID != expectedUserID || !activePeerUserIDPattern.MatchString(page.UserID) ||
		page.GeneratedAt.IsZero() || limit < 1 || limit > MaxUserActivePeerLimit ||
		page.Items == nil || len(page.Items) > limit {
		return false
	}
	_, offset := page.GeneratedAt.Zone()
	if offset != 0 {
	...
	for _, peer := range page.Items {
		if peer.TorrentID < 1 || !activePeerInfoHashPattern.MatchString(peer.InfoHashV1) ||
			peer.TotalSizeBytes < 1 || peer.ClientFamily == "" || len(peer.ClientFamily) > 32 ||
			(peer.AddressFamily != 4 && peer.AddressFamily != 6) || peer.Uploaded < 0 || peer.Downloaded < 0 ||
			peer.UploadSpeed < 0 || peer.DownloadSpeed < 0 || peer.Left < 0 || peer.Left > peer.TotalSizeBytes ||
			peer.LastAnnounce.IsZero() || peer.LastAnnounce.After(page.GeneratedAt) {
			return false
		}
		_, peerOffset := peer.LastAnnounce.Zone()
		if peerOffset != 0 {
