...
import { personalAPIKeyKeys } from "~/features/auth/api/personal-api-key.queries"
...
describe("PersonalAPIKeyPage", () => {
  afterEach(() => {
    vi.unstubAllGlobals()
  })

  it("keeps API Key in the old PtYes passkey position and issues one shared scoped key", async () => {
    const user = userEvent.setup()
    const queryClient = personalAPIKeyQueryClient()
    const fetchMock = vi
      .fn()
    ...
    expect(screen.getByLabelText("新个人 API Key")).toHaveValue(apiKey)
    expect(fetchMock).toHaveBeenCalledTimes(2)
    const issueRequest = fetchMock.mock.calls[0]?.[0] as Request
    await expect(issueRequest.clone().json()).resolves.toEqual({
      scopes: [
        "profile:read",
        "torrent:read",
        "torrent:download",
    ...
    expect(screen.queryByLabelText("新个人 API Key")).not.toBeInTheDocument()
    expect(screen.getByDisplayValue(/^pgk_A{8}•+$/)).toBeVisible()
  })
})

function personalAPIKeyQueryClient() {
  ...
  queryClient.setQueryData(personalAPIKeyKeys.status(userId), {
    active: false,
    scopes: [],
  })
  return queryClient
