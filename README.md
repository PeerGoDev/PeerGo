...
- `services/settlement` 已接通第一段可靠消费闭环：Tracker 与 Settlement 共用唯一 `tracker.announce.v1` canonical Go contract；独立 operator 命令创建或严格核对 `PEERGO_SETTLEMENT_V1` durable pull consumer，运行时只绑定既有 consumer。`AckExplicit + DeliverAll + MaxDeliver=-1` 保持完整证据，单实例按 stream sequence 拉取最多 64 条并在一个 PostgreSQL 事务内依次写入 unique inbox、session transition 和可选 raw interval，整批提交后才逐条执行确认式 `DoubleAck`；这只是摊薄 WAL commit 延迟，不会并行计算同一绝对 counter。PostgreSQL 暂时失败时对原批次发送 `InProgress` 并原地重试，任一条永久错误都会让整批 fail closed 且不自动 `Term`。相同 event ID/载荷只返回幂等结果，冲突载荷、错误 stream/subject/sequence 和损坏 session 均拒绝；counter 回退或 stopped 后重开会进入新 epoch，不生成负 delta；非递增事件时间记录为 `out_of_order` 且不移动可信 baseline。`ledger.raw_session_intervals` 只保存不可变原始事实，不提前套用 1× 或当前优惠。
