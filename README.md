...
- `/account/invitations` 已接通本人邀请资格、可用余额、签发、历史、邀请树/后宫/上家链和未领取凭证撤销；入口按 capability 显示。注册链接使用 `/register?invite=...` 预填一次性凭证，注册页读取后立即清理地址栏；明文不进入 TanStack Query 缓存，关闭签发结果窗口后不可找回。邀请注册强制使用有效凭证；开放注册允许直接注册，也会对主动携带的有效凭证执行相同的原子校验、消费并保留直接邀请关系。上文的 URL fragment 约束适用于邮箱验证、密码找回等服务端发出的敏感链接；成员主动复制分享的邀请链接使用 query 是明确例外，但注册页不会让它继续留在可见地址中。PtYes/Rousi 迁移会为每名用户保存不可变的可用邀请期初值，并恢复邀请码历史和邀请关系；只有切换时仍有效且未领取的旧邀请码保留 SHA-256 摘要以兼容原链接，已领取或过期记录不保留 bearer 摘要。旧邀请邮箱、连带处罚和旧魔力购买记录仍不迁入 Core。切换后的后宫加成按 Rousi 的直属一层、做种奖励 10%、至少做种 1 个、30 天活跃及每小时 100 魔力值上限计算；权益按小时封顶，但每 6 小时聚合成一条收款交易和一条结算凭证，页面同时展示当前预估、成员贡献、PeerGo 已结算值与已计入期初余额的旧站历史。
...
- `/social` 已按 Rousi 现有视觉语言实现“X 式信息流 + 轻社区板块”的组合布局：发布器可选择后台启用的板块并上传图片、创建投票或发放红包，关注/发现/热门使用真实服务端筛选与排序；右栏展示板块和话题聚合，动态卡片接通关注、点赞、评论、转发、投票、红包领取与打赏。后台 `/staff/content/social` 集中管理板块顺序、名称、图标、色调、成员发帖开关，以及动态移板、置顶、精选和隐藏，不生成假统计或假交互。
...
- 做种奖励、后宫加成、签到、连续签到和成员赠送已经接通；Rousi 成员赠送当前按零手续费启用。旧站累计/当前/最长连续签到及补签卡余额以不可变期初证据承接，补签卡的使用入口、商城和兑换仍按后续独立功能推进。
- 勋章购买/授予/佩戴/有效期及规则化加成。
- 种子、动态与评论的整数魔力值打赏以及动态红包已经接通；Rousi 内容打赏当前按 30% 手续费启用。工作组资金池、赞助、支付回调和退款仍按独立能力推进。
...
- 文字、图片、关联种子、可见性和草稿。
- 关注、粉丝、屏蔽、话题、精选和置顶。
- 统一评论、楼中楼、举报、内容打赏、点赞、转发和单选投票已经接通；表情回应和成员屏蔽仍待后续独立能力推进。
...
- `services/core/cmd/legacy-personal-state` 在种子 ID 映射完成后迁入收藏、已领取邀请关系、每名用户的可用邀请期初值、邀请码历史，以及按用户聚合的 `harem`/`invite_reward` 历史证据。9,840 条旧收藏全部写入不可变来源凭证，同一用户/种子的重复行只建立一次当前收藏；重跑只核对凭证，不会复活用户上线后主动删除的收藏。每个邀请余额只保留一条不可变 opening 和一条小型当前账户投影，之后每次签发/撤销各追加一条定额事件；过期不自动返还，撤销返还一个名额。全部旧邀请码保存来源指纹与状态，但只有切换时仍有效且未领取的 token 保留 SHA-256 摘要并进入注册凭证表，已领取/过期 token 与旧邮箱都不保留。可映射的邀请者/受邀者写入统一不可变关系图；源库中指向不存在邀请者的孤立关系保留为未映射证据，不虚构用户。后宫奖励已经包含在迁移的期初魔力值中，因此这里只保存精确合计、整数展示值、来源行数和时间范围，绝不二次入账。原生 PeerGo 邀请注册完成时也写入同一关系图。
...
- 动态圈互动、媒体、轻社区板块与后台管理首版已完成；继续推进成员屏蔽、表情回应、治理选举、赞助及其后台能力。
