...
2. 旧系统只用于确认“有哪些功能”，并离线导出用户主数据、种子元数据和原始 `.torrent` 文件；不迁移旧账目、行为记录或社区内容，导入数据映射到全新 schema。
...
- `services/settlement` 已接通第一段可靠消费闭环：Tracker 与 Settlement 共用唯一 `tracker.announce.v1` canonical Go contract；独立 operator 命令创建或严格核对 `PEERGO_SETTLEMENT_V1` durable pull consumer，运行时只绑定既有 consumer。首版以 `AckExplicit + DeliverAll + MaxDeliver=-1 + MaxAckPending=1` 保持绝对 counter 顺序，PostgreSQL 暂时失败时对同一 in-flight 消息发送 `InProgress` 并原地重试；只有 unique inbox、session transition 和可选 raw interval 在 Tracker Ledger 同一事务提交后才执行确认式 `DoubleAck`。相同 event ID/载荷只返回幂等结果，冲突载荷、错误 stream/subject/sequence 和损坏 session fail closed 且不自动 `Term`。counter 回退或 stopped 后重开会进入新 epoch，不生成负 delta；非递增事件时间记录为 `out_of_order` 且不移动可信 baseline。`ledger.raw_session_intervals` 只保存不可变原始事实，不提前套用 1× 或当前优惠。
...
- Tracker 最小 HTTP announce 与可靠发布纵切已经接通：Core 另行构建并签名 `tracker-subject-control/v1` 当前准入快照，Tracker 只在本地用共享 lookup key 对 32 位路由 passkey 做 domain-separated HMAC，不保存或记录明文。原生 `net/http` handler 严格解析 raw query、返回 bencode compact IPv4/IPv6 peers，并把连接 socket 地址作为唯一 endpoint；请求提供的 `ip/ipv4/ipv6` 不会覆盖它。固定分片、容量上限、dense peer vector、有界抽样和预算式过期清理组成当前单进程 Swarm Engine。每个成功响应先把带 UUIDv7、绝对 counter、可选稳定 completion identity 和两类快照精确 sequence 的 `tracker.announce.v1` 事件 fsync 到私有本地 WAL；独立 publisher 按序重放原始事件，只有取得 JetStream storage ACK 后才原子推进绑定 event ID 与 payload digest 的 checkpoint。进程崩溃最多导致同一 event ID 重放，不能越过未确认记录；完整 ACK 文件按 reset-checkpoint-before-truncate 顺序安全回收。Settlement 与 Core completion durable 各自消费该 stream，前者生成可靠流量账本，后者只投影真实完成跃迁。Swarm Engine 另以隐私安全的 info hash + seeders/leechers 完整快照进入独立 DiscardOld stream；快照不含 peer、用户或 endpoint。认证 scrape 已复用相同 passkey、控制快照、请求限流和动态 runtime policy，支持有界重复 `info_hash` 且禁止 full scrape；客户端族策略以及用户/地址 token bucket 也已经进入 announce/scrape 热路径。raw announce 与 scrape parser 均有 Go fuzz 门禁。Tracker 另在强制不同于公开数据端口的私网地址提供 Prometheus `/metrics`，只输出有界 action/result、地址族、已识别客户端族、事件类型、延迟、active swarm/peer 与 WAL 容量/积压，不含 passkey、用户/种子 ID、IP、peer ID、完整 info hash 或 raw query；切换告警规则位于 `deploy/observability/tracker-alerts.yaml`。WAL group commit、standby/rendezvous、热点分区、真实客户端二进制矩阵和峰值 loadgen 仍未接通。
...
- 明确不导入事件历史：完成记录、做种时间、H&R、收藏、评论、公告、举报、通知、警告、历次限制/处罚命令和任何历史审计记录。正式恢复只把最新快照中的当前封禁、下载限制、VIP、角色及用户资产状态建立为带来源证据的 PeerGo opening；账户申诉绑定当前 opening/version，不伪造旧站处罚时间线。
...
- `settlement.event_inbox`：以 `event_id` 做永久业务幂等键，同时约束原始 payload digest、stream/subject、stream sequence、delivery count 与终态 outcome；同一 stream sequence 不能属于两个事件。
...
- `ledger.raw_session_intervals`：按当前事件 ID 追加不可变相邻区间，保存前后绝对值、精确原始 delta、完成跃迁和事件时刻证据；它不是已优惠/已计费结果。

inbox fence、session 行锁/单调迁移和可选 raw interval 在同一事务完成。session 首次出现只建 baseline；counter 回退或 stopped 后再次出现会递增 epoch 且不产生负 delta；非递增事件时间保留为 `out_of_order` 证据但不移动 baseline。当前 inbox/raw interval 不执行自动删除，归档与分区必须先证明永久 event key、重投窗口和对账恢复路径，不能为了表大小提前破坏幂等。
...
- 当前 correctness-first adapter 先追加到私有本地 WAL 并逐条 fsync，才能返回成功；
  尚未用 group commit 换取吞吐；以后合并 fsync 也不能降低“成功响应前已经持久化”的语义。
...
- JetStream 与本地重放共同提供 at-least-once；服务端去重仅覆盖配置窗口。跨窗口与消费
  端重投的“恰好一次业务效果”仍必须由 Settlement 数据库 unique inbox 和同一事务保证。
...
```json
{
  "schema_version": "tracker.announce.v1",
  "event_id": "uuid-v7",
...
- consumer 从 stream 首条事件开始，使用 explicit ACK、无限 redelivery 和确认式 `DoubleAck`。数据库提交前绝不 ACK；ACK 确认丢失只会让相同 event ID 重投并命中 inbox。
- 首版 `MaxAckPending=1`，数据库暂时失败时在同一消息上发送 `InProgress` 并有界等待后重试，不 NAK 后越过当前消息。这是为了保持全局绝对 counter 顺序；只有引入稳定 session-key 分区和跨分区对账后才能提高并发。
...
迁移范围到此为止。用户头像、帖子/评论正文图片、无数据库引用的上传孤儿，以及促销 assignment、用户-种子进度、完成记录、做种/下载时长、H&R、收藏、评论、公告、举报、通知、处罚时间线、交易补偿和旧审计记录都不导入。用户当前流量、整数化魔力值/经验/等级/VIP/封禁/下载限制和勋章则只作为切换时刻 opening 迁入，不伪造历史流水。
...
当前专用工具只迁移本节声明的用户身份、用户 opening、勋章与种子数据，不兼任旧社区或事件历史导入器：
