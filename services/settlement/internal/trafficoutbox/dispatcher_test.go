...
func TestDispatcherMarksPublishedAfterStorageAck(t *testing.T) {
	t.Parallel()
	...
	dispatcher, err := NewDispatcher(repository, recordingPublisher{}, DispatcherConfig{}, func() time.Time { return now }, slog.New(slog.NewTextHandler(io.Discard, nil)))
	if err != nil {
		t.Fatal(err)
	}
...
func TestDispatcherKeepsOutboxForRetryWhenPublishFails(t *testing.T) {
	t.Parallel()
	now := time.Date(2026, time.August, 9, 15, 0, 0, 0, time.UTC)
	...
	processed, err := dispatcher.RunOnce(context.Background())
	if err != nil || !processed || !repository.released || repository.releaseCode != "traffic_publish_failed" || !repository.availableAt.Equal(now.Add(2*time.Second)) {
