...
func NewJetStreamPublisher(client publishClient, stream, subject string) (*JetStreamPublisher, error) {
	if client == nil || !settlementtrafficv1.ValidStreamName(stream) || !settlementtrafficv1.ValidLiteralSubject(subject) {
		return nil, ErrInput
	}
...
func (publisher *JetStreamPublisher) Publish(ctx context.Context, pending PendingEvent) error {
	if pending.EventID.String() != pending.Event.EventID || len(pending.Payload) == 0 || settlementtrafficv1.Validate(pending.Event) != nil {
		return ErrInput
	}
	...
	if err != nil {
		return fmt.Errorf("publish Settlement traffic event to JetStream: %w", err)
	}
