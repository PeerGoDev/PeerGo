package main

import (
	"context"
	"flag"
	"fmt"
	"log/slog"
	"os"
	"strings"
	"time"

	"github.com/jackc/pgx/v5/pgxpool"
	"github.com/peergo/peergo/services/settlement/internal/config"
	"github.com/peergo/peergo/services/settlement/internal/operatorinput"
	platformpostgres "github.com/peergo/peergo/services/settlement/internal/platform/postgres"
	"github.com/peergo/peergo/services/settlement/internal/policy"
	"github.com/peergo/peergo/services/settlement/internal/settler"
	"github.com/peergo/peergo/services/settlement/internal/timeline"
)

func main() {
	logger := slog.New(slog.NewJSONHandler(os.Stdout, nil))
	if err := run(logger, os.Args[1:]); err != nil {
		logger.Error("Settlement policy timeline append failed", "error", err)
		os.Exit(1)
	}
}

// run is intentionally an operator-only control-plane tool. It persists an
// already compiled complete policy snapshot; it does not expose a shortcut for
// editing raw promotion settings or recomputing historical traffic.
func run(logger *slog.Logger, args []string) error {
	flags := flag.NewFlagSet("policy-timeline-append", flag.ContinueOnError)
	flags.SetOutput(os.Stderr)
	entryID := flags.String("id", "", "immutable timeline entry UUID")
	snapshotFile := flags.String("snapshot-file", "", "canonical compiled snapshot JSON file")
	effectiveAt := flags.String("effective-at", "", "RFC3339 effective timestamp")
	userID := flags.String("user-id", "", "optional exact user UUID selector")
	torrentID := flags.String("torrent-id", "", "optional exact torrent numeric ID selector")
	torrentSequence := flags.String("torrent-control-sequence", "", "optional exact Tracker torrent control sequence")
	subjectSequence := flags.String("subject-control-sequence", "", "optional exact Tracker subject control sequence")
	if err := flags.Parse(args); err != nil {
		return err
	}
	revision, err := parseRevision(*entryID, *snapshotFile, *effectiveAt, *userID, *torrentID, *torrentSequence, *subjectSequence)
	if err != nil {
		return err
	}
	settings, err := config.LoadTrackerLedgerProcess()
	if err != nil {
		return err
	}
	ctx, cancel := context.WithTimeout(context.Background(), 15*time.Second)
	defer cancel()
	pool, err := pgxpool.New(ctx, settings.DatabaseURL)
	if err != nil {
		return fmt.Errorf("open Tracker Ledger database: %w", err)
	}
	defer pool.Close()
	if err := pool.Ping(ctx); err != nil {
		return fmt.Errorf("ping Tracker Ledger database: %w", err)
	}
	if err := platformpostgres.RequireCurrentMigration(ctx, pool); err != nil {
		return err
	}
	repository, err := settler.NewPostgresRepository(pool)
	if err != nil {
		return err
	}
	created, err := repository.AppendRevision(ctx, revision, time.Now().UTC())
	if err != nil {
		return err
	}
	logger.Info("Settlement immutable policy timeline revision verified", "id", revision.ID, "created", created, "effective_at", revision.EffectiveAt)
	return nil
}

func parseRevision(idValue, snapshotFile, effectiveAtValue, userIDValue, torrentIDValue, torrentSequenceValue, subjectSequenceValue string) (timeline.Revision, error) {
	id, err := operatorinput.RequiredUUID(idValue, "--id")
	if err != nil {
		return timeline.Revision{}, err
	}
	effectiveAt, err := operatorinput.RequiredTime(effectiveAtValue, "--effective-at")
	if err != nil {
		return timeline.Revision{}, err
	}
	encoded, err := os.ReadFile(strings.TrimSpace(snapshotFile))
	if err != nil {
		return timeline.Revision{}, fmt.Errorf("read --snapshot-file: %w", err)
	}
	snapshot, err := policy.DecodeSnapshot(encoded)
	if err != nil {
		return timeline.Revision{}, fmt.Errorf("decode --snapshot-file: %w", err)
	}
	scope, err := operatorinput.ParseScope(operatorinput.ScopeValues{
		UserID: userIDValue, TorrentID: torrentIDValue,
		TorrentControlSequence: torrentSequenceValue, SubjectControlSequence: subjectSequenceValue,
	})
	if err != nil {
		return timeline.Revision{}, err
	}
	revision := timeline.Revision{ID: id, Scope: scope, EffectiveAt: effectiveAt, Snapshot: snapshot}
	if err := timeline.ValidateRevision(revision); err != nil {
		return timeline.Revision{}, err
	}
	return revision, nil
}
