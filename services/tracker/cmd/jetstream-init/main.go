...
func main() {
	...
	if err := run(logger); err != nil {
		logger.Error("Tracker announce stream provisioning failed", "error", err)
		os.Exit(1)
	}
}

func run(logger *slog.Logger) error {
	settings, err := config.LoadStreamProvisioner()
	if err != nil {
		return fmt.Errorf("load stream provisioner configuration: %w", err)
	}
	...
	logger.Info("Tracker announce stream configuration verified", "stream", settings.Stream.Name, "created", created)
	return nil
}
