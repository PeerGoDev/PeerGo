...
func NewRunner(evaluator Evaluator, failures FailureRecorder, interval time.Duration, batch int, logger *slog.Logger, now func() time.Time) (*Runner, error) {
	if evaluator == nil || failures == nil || interval < 10*time.Second || interval > time.Hour ||
		batch < 1 || batch > MaximumWorkerBatch || logger == nil {
		return nil, ErrInput
	}
	if now == nil {
		now = time.Now
...
// Run evaluates immediately and then on a bounded interval. A single failed
// database transaction is recorded and retried on the next tick; it does not
// stop promotion or H&R delivery goroutines in the shared Core policy worker.
...
func (runner *Runner) runOnce(ctx context.Context) {
	...
	if err != nil {
		...
		runner.logger.Error("ratio watch evaluation failed", "error", err)
		return
	}
	if result.Skipped {
		return
	}
	runner.logger.Info("ratio watch evaluation completed",
