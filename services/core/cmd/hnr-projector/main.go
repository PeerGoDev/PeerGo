...
import (
	...
	"github.com/peergo/peergo/services/core/internal/hnrconsumer"
...
func main() {
	...
	if err := run(logger); err != nil {
		logger.Error("Core H&R projector stopped", "error", err)
		os.Exit(1)
	}
}

func run(logger *slog.Logger) error {
	settings, err := config.LoadHNRProjector()
	if err != nil {
		return fmt.Errorf("load Core H&R projector configuration: %w", err)
	}
	...
	connection, js, err := trafficconsumer.Connect(settings.NATS, "peergo-core-hnr-projector", logger)
	if err != nil {
		return fmt.Errorf("connect Core H&R JetStream consumer: %w", err)
	}
	...
	if err != nil {
		return fmt.Errorf("open Core H&R JetStream source: %w", err)
	}
	projector, err := traffic.NewPostgresRepository(pool, time.Now)
	if err != nil {
		return fmt.Errorf("compose Core H&R repository: %w", err)
	}
	runner, err := hnrconsumer.NewRunner(source, projector, hnrconsumer.RunnerConfig{
		Stream: settings.Stream, Subject: settings.Subject, Durable: settings.Durable,
		ProcessTimeout: settings.ProcessTimeout, AckTimeout: settings.AckTimeout, RetryDelay: settings.RetryDelay,
	}, time.Now, logger)
	if err != nil {
		return fmt.Errorf("compose Core H&R projector: %w", err)
	}
	logger.Info("Core H&R projector started", "stream", settings.Stream, "subject", settings.Subject, "durable", settings.Durable)
	...
	if err := connection.FlushWithContext(shutdownCtx); err != nil && !errors.Is(err, context.Canceled) {
		runtimeErr = errors.Join(runtimeErr, fmt.Errorf("flush Core H&R NATS connection: %w", err))
	}
	return runtimeErr
}
