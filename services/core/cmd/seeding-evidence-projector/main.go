...
import (
	...
	"github.com/peergo/peergo/services/core/internal/seedingconsumer"
...
func main() {
	...
	if err := run(logger); err != nil {
		logger.Error("Core seeding evidence projector stopped", "error", err)
		os.Exit(1)
	}
}

func run(logger *slog.Logger) error {
	settings, err := config.LoadSeedingEvidenceProjector()
	if err != nil {
		return fmt.Errorf("load Core seeding evidence projector configuration: %w", err)
	}
	...
	connection, js, err := trafficconsumer.Connect(settings.NATS, "peergo-core-seeding-evidence-projector", logger)
	if err != nil {
		return fmt.Errorf("connect Core seeding evidence JetStream consumer: %w", err)
	}
	...
	if err != nil {
		return fmt.Errorf("open Core seeding evidence JetStream source: %w", err)
	}
	projector, err := seedingreward.NewPostgresEvidenceRepository(pool, time.Now, settings.MaxFutureSkew)
	if err != nil {
		return fmt.Errorf("compose Core seeding evidence repository: %w", err)
	}
	runner, err := seedingconsumer.NewRunner(source, projector, seedingconsumer.RunnerConfig{
		Stream: settings.Stream, Subject: settings.Subject, Durable: settings.Durable,
		ProcessTimeout: settings.ProcessTimeout, AckTimeout: settings.AckTimeout, RetryDelay: settings.RetryDelay,
	}, time.Now, logger)
	if err != nil {
		return fmt.Errorf("compose Core seeding evidence projector: %w", err)
	}
	logger.Info("Core seeding evidence projector started", "stream", settings.Stream, "subject", settings.Subject, "durable", settings.Durable)
	...
	if err := connection.FlushWithContext(shutdownCtx); err != nil && !errors.Is(err, context.Canceled) {
		runtimeErr = errors.Join(runtimeErr, fmt.Errorf("flush Core seeding evidence NATS connection: %w", err))
	}
	return runtimeErr
}
